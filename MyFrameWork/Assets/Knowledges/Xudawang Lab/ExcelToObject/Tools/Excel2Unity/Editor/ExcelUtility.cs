    ...
    public void ConvertToJson(string JsonPath, Encoding encoding)
    {
        ...
        for (int i = 1; i < rowCount; i++)
        {
            ...
            for (int j = 0; j < colCount; j++)
            {
                //读取第1行数据作为表头字段
                string field = mSheet.Rows[0][j].ToString();
                string exchangeFielName = string.Empty;
                switch (field)
                {
                    case "产品系列":
                        exchangeFielName = "productSeries";
                        break;

                    case "颜色":
                        exchangeFielName = "color";
                        break;

                    case "产品名称":
                        exchangeFielName = "productNameCHS";
                        break;

                    case "产品名称（英文）":
                        exchangeFielName = "productNameEn";
                        break;
                    ...
                    case "产品编码":
                        exchangeFielName = "productNum";
                        break;

                    case "宽度(mm)":
                        exchangeFielName = "width";
                        break;

                    case "深度(mm)":
                        exchangeFielName = "depth";
                        break;

                    case "价格(元/m^2)":
                        exchangeFielName = "price";
                        break;

                    case "有无贴图":
                        exchangeFielName = "matType";
                        break;

                    default:
                        Debug.Log("无法查找到的属性名：" + field);
                        break;
                }

                if (string.IsNullOrEmpty(exchangeFielName))
                {
                    continue;
                }
                ...
                row[exchangeFielName] = mSheet.Rows[i][j];
            }
            if (row.Count > 0)
            {
                //添加到表数据中
