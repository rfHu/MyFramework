...
namespace RHFramework
{
    public class PETimer
    {
        ...
        public int AddTimeTask(Action<int> callback, double delay, PETimeUnit timeUnit = PETimeUnit.Millisecond, int count = 1)
        {
            ...
            lock (lockTime)
            {
                tmpTimeLst.Add(new PETimeTask(tid, callback, nowTime + delay / timeMutiNum, delay / timeMutiNum, count));
            }
            return tid;
        }
        ...
        public bool ReplaceTimeTask(int tid, Action<int> callback, float delay, PETimeUnit timeUnit = PETimeUnit.Millisecond, int count = 1)
        {
            ...
            nowTime = GetUTCMilliseconds();
            PETimeTask newTask = new PETimeTask(tid, callback, nowTime + delay / timeMutiNum, delay / timeMutiNum, count);
        ...
        public bool ResetTimeTaskCount(int tid, int count)
        {
            bool reset = false;
            for (int i = 0; i < taskTimeLst.Count; i++)
            {
        ...
        public bool PauseTimeTask(int tid, bool pause)
        {
            bool finish = false;
            for (int i = 0; i < taskTimeLst.Count; i++)
            {
                if (taskTimeLst[i].tid == tid)
                {
                    taskTimeLst[i].pause = pause;
                    if (pause)
                    {
                        taskTimeLst[i].pauseTime = GetUTCMilliseconds();
                    }
                    else
                    {
                        var nowTime = GetUTCMilliseconds();
                        var offset = nowTime - taskTimeLst[i].pauseTime;
                        taskTimeLst[i].destTime += offset;
                    }
                    finish = true;
                    break;
                }
            ...
            if (!finish)
            {
                for (int i = 0; i < tmpTimeLst.Count; i++)
                {
                    if (tmpTimeLst[i].tid == tid)
                    {
                        tmpTimeLst[i].pause = pause;
                        if (pause)
                        {
                            tmpTimeLst[i].pauseTime = GetUTCMilliseconds();
                        }
                        else
                        {
                            var nowTime = GetUTCMilliseconds();
                            var offset = nowTime - taskTimeLst[i].pauseTime;
                            tmpTimeLst[i].destTime += offset;
                        }
                        finish = true;
                        break;
                    }
        ...
        public void PauseAllTimeTask(bool pause)
        {
            for (int i = 0; i < taskTimeLst.Count; i++)
            {
                taskTimeLst[i].pause = pause;
        ...
        private float timeMutiNum = 1;
        public void MutiSpeedChangeAllTask(float mutiNum = 1)
        {
            if (mutiNum <= 0)
            {
                LogInfo("警告!!倍速不能小于0");
            ...
            var muti = mutiNum / timeMutiNum;
            timeMutiNum = mutiNum;
            for (int i = 0; i < taskTimeLst.Count; i++)
            {
                taskTimeLst[i].delay /= muti;
                taskTimeLst[i].destTime = GetUTCMilliseconds() + (taskTimeLst[i].destTime - GetUTCMilliseconds()) / muti;
            }

            for (int i = 0; i < tmpTimeLst.Count; i++)
            {
                tmpTimeLst[i].delay /= muti;
                tmpTimeLst[i].destTime = GetUTCMilliseconds() + (tmpTimeLst[i].destTime - GetUTCMilliseconds()) / muti;
            }
        }
        public float GettimeMutiNum()
        {
            return timeMutiNum;
        }
        #endregion
